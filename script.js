(function() {
    ...
    // GERAR RELATÓRIO NA TELA (COM GRÁFICO BONITO)
    ...
    // IMPRIMIR RELATÓRIO (APENAS DADOS, SEM GRÁFICO, COM COLUNAS ORGANIZADAS)
    ...
    imprimirRemoverBtn.onclick = window.imprimirRelatorio;
    codigoInput.focus();
})();(function() {
    // CONFIGURAÇÃO
    const SUPABASE_URL = 'https://zqmvpqvkvyuvuatbsrlv.supabase.co';
    ...
    // IMPRIMIR RELATÓRIO (COM GRÁFICO E TABELA ORGANIZADA)
    window.imprimirRelatorio = async function() {
        // Buscar dados
        const response = await fetch(`${SUPABASE_URL}/rest/v1/produtos_validade?select=*`, {
            headers: { 'apikey': SUPABASE_KEY, 'Authorization': `Bearer ${SUPABASE_KEY}` }
