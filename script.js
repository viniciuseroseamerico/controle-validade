...
const loginErroDiv = document.getElementById('loginErro');
const lojaAtualSpan = document.getElementById('lojaAtual');
...
// SESSÃO ATUAL (preenchida após login)
let lojaId = null;
let lojaNome = null;
...
function verificarSessao() {
    const sessao = localStorage.getItem('sessaoLoja');
    if (sessao) {
        const dados = JSON.parse(sessao);
        lojaId = dados.loja_id;
        lojaNome = dados.nome_loja;
        mostrarApp();
    } else {
        mostrarLogin();
    }
...
function mostrarApp() {
    ...
    lojaAtualSpan.textContent = `🏬 Loja: ${lojaNome}`;
    codigoInput.focus();
}
...
async function fazerLogin() {
    ...
    loginErroDiv.textContent = '';
    if (!usuario || !senha) {
        loginErroDiv.textContent = 'Preencha usuário e senha.';
        return;
    }
    loginBtn.disabled = true;
    try {
        const response = await fetch(
            `${SUPABASE_URL}/rest/v1/usuarios?usuario=eq.${encodeURIComponent(usuario)}&senha=eq.${encodeURIComponent(senha)}&ativo=eq.true&select=*`,
            { headers: { 'apikey': SUPABASE_KEY, 'Authorization': `Bearer ${SUPABASE_KEY}` } }
        );
        const dados = await response.json();
        if (dados.length === 0) {
            loginErroDiv.textContent = '❌ Usuário ou senha inválidos.';
        ...
        const user = dados[0];
        lojaId = user.loja_id;
        lojaNome = user.nome_loja;
        localStorage.setItem('sessaoLoja', JSON.stringify({ loja_id: lojaId, nome_loja: lojaNome }));
        loginUsuarioInput.value = '';
        loginSenhaInput.value = '';
        mostrarApp();
    } catch (e) {
        loginErroDiv.textContent = 'Erro ao conectar: ' + e.message;
    } finally {
...
loginSenhaInput.onkeypress = (e) => { if (e.key === 'Enter') fazerLogin(); };
loginUsuarioInput.onkeypress = (e) => { if (e.key === 'Enter') loginSenhaInput.focus(); };
...
// ============================================
// BUSCAR PRODUTO (sempre filtrado pela loja logada)
// ============================================
async function buscarProduto(ean) {
    loadingDiv.style.display = 'block';
    resultadoDiv.innerHTML = '';
    try {
        const response = await fetch(`${SUPABASE_URL}/rest/v1/produtos_validade?eam=eq.${ean}&loja_id=eq.${lojaId}&select=*`, {
            headers: { 'apikey': SUPABASE_KEY, 'Authorization': `Bearer ${SUPABASE_KEY}` }
        });
        const produtos = await response.json();
        loadingDiv.style.display = 'none';
        if (produtos.length === 0) {
            resultadoDiv.innerHTML = `<div class="nao-encontrado">⚠️ PRODUTO NÃO CADASTRADO NESTA LOJA!</div>
...
// ============================================
// IMPRIMIR (sempre filtrado pela loja logada)
// ============================================
async function imprimirRelatorio() {
    ...
    let html = `<h2>📊 RELATÓRIO — ${lojaNome}</h2><p>Data: ${new Date().toLocaleDateString()}</p><p>Total: ${produtos.length}</p><hr>`;
    html += `<h3>🔴 VENCIDOS</h3>`;
    if (vencidos.length) {
